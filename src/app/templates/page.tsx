...
export default function MessageTemplatesPage() {
  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      setFeedback({ type: 'error', message: err.message || '載入通訊範本失敗' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    if (!content.trim()) {
      setFeedback({ type: 'error', message: '請輸入通訊範本內容' });
      return;
    }
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <p className="text-xs text-slate-500 mb-6">系統將自動產生唯一訊息代碼，並允許您選取變數標籤組合出標準提醒文案。</p>

          <form onSubmit={handleSaveTemplate} className="space-y-5" noValidate>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1">
                  訊息代碼 (Message Key) <span className="text-xs font-normal text-sky-800">(系統自動生成・唯讀)</span>
                </label>
                <input
                  type="text"
                  readOnly
            ...
            <div className="space-y-2 bg-sky-50/70 p-4 rounded-xl border border-sky-200">
              <span className="text-xs font-bold text-sky-950 block">點擊下方變數標籤，即可於游標處或文末快速插入動態資料：</span>
                ...
                {loading ? (
                  ...
                  templates.map((t) => (
                    <tr key={t.message_key} className="hover:bg-sky-50/40">
                      ...
                      <td className="px-4 py-3 text-xs text-slate-600 whitespace-pre-line font-mono max-w-xl">{t.content}</td>
                    </tr>
                  ))
                )}
              </tbody>
