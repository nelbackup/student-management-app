      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-slate-200 gap-4">
          <div className="flex items-center gap-4">
            <div className="relative w-14 h-14 sm:w-16 sm:h-16 bg-white rounded-full shadow-md border-2 border-amber-400 p-1">
          ...
          <Link href="/student" className="px-4 py-2 text-sm font-semibold text-sky-900 bg-white hover:bg-sky-50 rounded-xl border border-sky-200 shadow-sm transition">返回學員管理</Link>
        </div>
        ...
        {feedback && (
          <div className={`p-4 rounded-xl text-sm font-medium border flex items-center gap-2 ${feedback.type === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-rose-50 text-rose-800'}`}>
            <span>{feedback.type === 'success' ? '✅' : '⚠️'}</span><span>{feedback.message}</span>
          </div>
        )}
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleSaveTemplate} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1">訊息代碼 (自動生成・唯讀)</label>
                <input type="text" readOnly disabled value={nextMessageKey} className="w-full px-3.5 py-2.5 border rounded-xl text-sm font-mono font-bold bg-slate-100" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1">範本標題名稱 *</label>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="例如：上課前溫馨提示" className="w-full px-3.5 py-2.5 border rounded-xl text-sm" />
              </div>
            </div>
            ...
            <div className="space-y-2 bg-sky-50/70 p-4 rounded-xl border border-sky-200">
              ...
              <div className="flex flex-wrap gap-2">
                {AVAILABLE_VARIABLES.map((v) => (
                  <button key={v.tag} type="button" onClick={() => handleInsertVariable(v.tag)} className="px-3 py-1.5 bg-white hover:bg-sky-100 text-sky-900 border rounded-lg text-xs font-mono font-bold shadow-sm cursor-pointer">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-bold text-slate-800">訊息內容 * (上限 500 字)</label>
                <span className="font-mono text-xs font-bold text-slate-500">{content.length} / 500 字</span>
              </div>
              <textarea rows={6} maxLength={500} value={content} onChange={(e) => setContent(e.target.value)} placeholder="家長您好~~~..." className="w-full p-3.5 border rounded-xl text-sm font-mono leading-relaxed" />
            </div>

            <button type="submit" disabled={saving} className="w-full py-3 bg-sky-950 hover:bg-sky-900 text-white font-bold rounded-xl text-sm shadow cursor-pointer">{saving ? '儲存中...' : '儲存新預設通訊範本'}</button>
          </form>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
          <div className="p-4 bg-sky-50 font-bold text-sm text-sky-950">現有預設通訊範本清單 ({templates.length})</div>
          <table className="min-w-full divide-y text-sm">
            <thead className="bg-sky-950 text-white text-xs uppercase">
              <tr><th className="px-4 py-3 text-left">代碼</th><th className="px-4 py-3 text-left">標題</th><th className="px-4 py-3 text-left">內容預覽</th></tr>
            </thead>
